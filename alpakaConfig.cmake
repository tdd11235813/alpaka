...
OPTION(ALPAKA_ACC_HIP_ENABLE "Enable the HIP back-end" OFF)
...
IF(ALPAKA_ACC_HIP_ENABLE AND ALPAKA_ACC_GPU_CUDA_ENABLE)
    MESSAGE(FATAL_ERROR "CUDA and HIP cannot be enabled both at the same time.")
ENDIF()

# no clang for HIP at the moment
IF(ALPAKA_ACC_HIP_ENABLE AND CMAKE_CXX_COMPILER_ID MATCHES "Clang")
    MESSAGE(WARNING "Optional alpaka dependency HIP can not be built with clang! HIP back-end disabled!")
    SET(ALPAKA_ACC_HIP_ENABLE OFF CACHE BOOL "Enable the HIP GPU back-end" FORCE)
ENDIF()

# HIP is only supported on Linux
IF(ALPAKA_ACC_HIP_ENABLE AND (MSVC OR WIN32))
...
IF(ALPAKA_ACC_CPU_B_OMP2_T_SEQ_ENABLE OR ALPAKA_ACC_CPU_B_SEQ_T_OMP2_ENABLE OR ALPAKA_ACC_CPU_BT_OMP4_ENABLE)
    ...
    ELSE()
        ...
        IF(NOT MSVC AND NOT ALPAKA_ACC_HIP_ENABLE)
    ...
    ELSE()
        ...
        ELSE()
            ...
            IF(HIP_PLATFORM MATCHES "nvcc")
                ...
                IF(CUDA_VERSION VERSION_LESS 9.0)
                    SET(ALPAKA_HIP_ARCH "20" CACHE STRING "GPU architecture" FORCE)
                ELSE()
                    SET(ALPAKA_HIP_ARCH "30" CACHE STRING "GPU architecture" FORCE)
                ENDIF()
...
ENDIF()
IF(ALPAKA_ACC_HIP_ENABLE)
    LIST(APPEND _ALPAKA_COMPILE_DEFINITIONS_PUBLIC "ALPAKA_ACC_HIP_ENABLED")
    MESSAGE(STATUS ALPAKA_ACC_HIP_ENABLED)
ENDIF()
