...
(cd "${ROCRAND_SOURCE_DIR}"; mkdir -p build; cd build; cmake -DCMAKE_BUILD_TYPE="${CMAKE_BUILD_TYPE}" -DCMAKE_INSTALL_PREFIX=${ALPAKA_CI_HIP_ROOT_DIR} -DBUILD_BENCHMARK=OFF -DBUILD_TEST=OFF -DNVGPU_TARGETS="--gpu-architecture=sm_30" -DCMAKE_MODULE_PATH=${ALPAKA_CI_HIP_ROOT_DIR}/cmake .. && make && make install)
